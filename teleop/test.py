...
def main():
    ...
    oef_beta = 1.5
    oef = None
    # client = RobotClient(namespace="gr/fourier-grasper", server_ip="192.168.137.123")
    ...
    # Load the .npy file with 4x4 transformation matrices
    right_pose_data = np.load('right_angles.npy')  # Assuming it contains Nx4x4 matrices
    
    # Initialize the Meshcat visualizer
    viz = start_meshcat_visualizer(gr1kinematics.robot)
    if viz is None:
        print("Meshcat visualizer not started")
        exit(1)
    ...
    try:
        ...
        print("Meshcat frames created successfully")
    except Exception as e:
        print(f"Error creating Meshcat frames: {e}")
        exit(1)
    
    # Fixed target for the left hand
    ...
    # Process each frame in the recorded data

    q_pos = np.zeros(32) 

    right_transform = right_pose_data[0]
    ...
    viewer["right_ee_transform_target"].set_transform(gr1kinematics.get_transform("link_torso", "base") @ right_transform_corrected)
    # Visualize the left hand's fixed target
    viewer["left_ee_transform_target"].set_transform(gr1kinematics.get_transform("link_torso", "base") @ left_target)
    # viewer["left_ee_transform_target"].set_transform(left_target)
    # viewer["right_ee_transform_target"].set_transform(right_transform_corrected)
    # Perform inverse kinematics for both hands
    target_q_start = gr1kinematics.inverse_kinematics(q=q_pos, l_target =left_target, r_target=right_transform_corrected)
    target_q_start = target_q_start * 180.0 / np.pi
    viewer["right_ee_transform_target"].set_transform(gr1kinematics.get_transform("link_torso", "base") @ right_transform_corrected)
    ...
    # dynamic_move(client, start_pos, target_q_start[7:], 5.0)

    # start_pos_left = client.joint_positions.copy()[18:25] 
    # dynamic_move(client, start_pos_left, first_place_left, 5.0)

    frame_count = right_pose_data.shape[0]  # Total number of frames
    for i in range(frame_count-1):
        # Extract the 4x4 transformation matrix for the current frame
        right_transform = right_pose_data[i+1]
        ...
        # Perform inverse kinematics for both hands
        q_total = gr1kinematics.inverse_kinematics(q=q_pos, l_target =left_target, r_target=right_transform_corrected)
        # print(target_q)
        # q_total = gr1kinematics.configuration.q
        if oef is None:
            oef = OneEuroFilter(time.time(), q_total,
                                min_cutoff=oef_min_cutoff, beta=oef_beta)
        ...
        # ik_result = filtered_pos * 180.0 / np.pi

        # client.move_joints(ControlGroup.RIGHT_ARM, ik_result[7:], 0.0)
        # client.move_joints(ControlGroup.LEFT_ARM, ik_result[:7], 0.0)

        # Sleep for the control period
        # gr1kinematics.rate.sleep()
        time.sleep(1/FREQUENCY_TELE)
